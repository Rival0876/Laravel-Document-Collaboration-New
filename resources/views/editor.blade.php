    ...
    window.APP_CONFIG = {
        ...
        user: {
            ...
            <div class="w-full md:w-1/4 bg-gray-50 p-4 rounded-lg shadow-sm border h-fit">
                ...
                <div id="version-container" class="space-y-3">
                @if($versions->isEmpty())
                    <p class="text-sm text-gray-500">Belum ada riwayat penyimpanan.</p>
                @else
                    <ul class="space-y-3">
                        @foreach($versions as $version)
    <div class="mb-3 p-3 bg-white border border-gray-200 rounded-lg shadow-sm hover:bg-indigo-50 cursor-pointer transition version-item" data-content="{{ $version->content }}">
